...
export type StudentProgressOverviewCardsVariant = "full" | "streak-only";

interface StudentProgressOverviewCardsProps {
  snapshot: StudentProgressSnapshot;
  variant?: StudentProgressOverviewCardsVariant;
}

function DayStreakCard({ snapshot }: { snapshot: StudentProgressSnapshot }) {
  return (
    <Card>
...
export function StudentProgressOverviewCards({
  ...
  if (variant === "streak-only") {
    return (
      <div className="max-w-md">
        <DayStreakCard snapshot={snapshot} />
      </div>
    );
  }
