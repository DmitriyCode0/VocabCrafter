...
async function waitForGeminiFileActive(fileName: string) {
  ...
  throw new Error("Timed out while Gemini prepared the lesson recording");
}
...
export async function POST(
  ...
  if (access.role !== "tutor") {
    return NextResponse.json(
      { error: "Only tutors can transcribe lesson recordings" },
      { status: 403 },
    );
  }
  ...
  const { data: recording, error: recordingError } = await supabaseAdmin
    ...
    .select("id, lesson_id, session_id, storage_bucket, storage_path")
