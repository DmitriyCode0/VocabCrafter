...
const EN_MESSAGES = {
  ...
  tutorProgressPage: {
    ...
    consecutiveDays: (count: number) =>
      `${count} ${formatPluralizedCount("en", count, { one: "consecutive day", other: "consecutive days" })}`,
...
const UK_MESSAGES: AppMessages = {
  ...
  tutorProgressPage: {
    ...
    consecutiveDays: (count: number) =>
      `${count} ${formatPluralizedCount("uk", count, { one: "день поспіль", few: "дні поспіль", many: "днів поспіль" })}`,
    uniqueWords: "Унікальні слова",
    masteredWords: (count: number) =>
      `${count} ${formatPluralizedCount("uk", count, { one: "засвоєне слово", few: "засвоєні слова", many: "засвоєних слів" })}`,
